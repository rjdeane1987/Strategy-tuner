...
st.title("📈 Strategy Tuner Pro – Regime-Based Long/Short with HMM, Hurst & Spectral")
...
trend_lev = st.sidebar.slider("Trend Regime Leverage", 0.0, 5.0, 1.0, 0.1)
chop_lev = st.sidebar.slider("Chop Regime Leverage", 0.0, 5.0, 1.0, 0.1)
vol_targeting = st.sidebar.checkbox("Enable Vol Targeting", value=False)
target_vol = st.sidebar.slider("Volatility Target", 0.05, 0.5, 0.15, 0.01)

@st.cache_data
def fetch_data(ticker, start, end):
    df = yf.download(ticker, start=start, end=end)["Adj Close"].to_frame("price")
...
window = 100
df["hurst"] = df["returns"].rolling(window).apply(compute_hurst, raw=False)
df["spectral"] = df["returns"].rolling(window).apply(compute_spectral_slope, raw=False)
df = df.dropna()
