...
df["hurst"] = df["returns"].rolling(100).apply(compute_hurst, raw=False)
df["spectral"] = df["returns"].rolling(100).apply(compute_spectral_slope, raw=False)
df = df.dropna()

obs = df[["hurst", "spectral"]].dropna().values

if len(obs) < 10:
    st.error("❗ Not enough data points to train the Hidden Markov Model. Try adjusting your date range, asset, or indicator filters.")
    st.stop()
...
# Filter signals by Hurst & Spectral ranges
